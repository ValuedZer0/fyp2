...
def load_dataset(name):
    ...
    if name == 'iris':
        ...
        df = pd.read_csv(_data_path('glass.data'), header=None)
        ...
        df = pd.read_csv(_data_path('balance-scale.data'), header=None)
        ...
        df = pd.read_csv(_data_path('column_3C.dat'), sep=r'\s+', header=None)
        ...
        df = pd.read_csv(_data_path('ecoli.data'), sep=r'\s+', header=None)
        ...
        df = pd.read_csv(_data_path('transfusion.data'))
        X = df.iloc[:, :4].values.astype(float)
        y = df.iloc[:, -1].values.astype(int)

    elif name == 'seeds':
        # 8 columns: 7 features + class (1,2,3)
        df = pd.read_csv(_data_path('seeds_dataset.txt'), sep=r'\s+', header=None)
    ...
    if np.isnan(X).any():
        raise ValueError(f"NaN in features of {name}")
    if np.isnan(y).any():
        raise ValueError(f"NaN in labels of {name}")
