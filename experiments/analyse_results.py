...
best_per_ds_out = best_per_ds[['dataset', 'outlier_method', 'norm', 'metric',
                               'ari_mean', 'ari_min', 'ari_max', 'ari_std',
             'acc_mean', 'acc_min', 'acc_max', 'acc_std', 'purity_mean', 'purity_std',
             'silhouette_mean', 'silhouette_min', 'silhouette_max', 'silhouette_std']]
...
# ------------------------------------------------------------------
# TOP COUPLES – best overall configurations (full 9/9 coverage)
# ------------------------------------------------------------------
combo_group = full_df.groupby(['outlier_method', 'norm', 'metric'])
acc_mean = combo_group['ari_mean'].mean()
...
for rank, (_, row) in enumerate(fully_covered.head(20).iterrows(), start=1):
    print(f"  {rank}. {row['outlier_method']:15s} {row['norm']:10s} {row['metric']:15s}  "
          f"Hungarian acc = {row['ari_mean']:.4f}")

# ------------------------------------------------------------------
# Per‑dataset Hungarian accuracy for the TOP-1 (PROPOSED) configuration
# ------------------------------------------------------------------
...
print("\nPer‑dataset Hungarian accuracy (PROPOSED):")
...
print(f"\nOverall Hungarian accuracy (PROPOSED): {overall_acc:.2f} ± {overall_acc_std:.2f}")